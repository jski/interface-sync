...
function IM:AttachViewerVendor()
	...
	if (frame:GetLeft() < MerchantFrame:GetRight()) and (frame:GetTop()<MerchantFrame:GetTop() and frame:GetTop()>MerchantFrame:GetBottom()) then
		ZGV.db.profile.frame_anchor_prevendor = {ZGV.Frame:GetParent():GetPoint()}
		ZGV.Frame:GetParent():ClearAllPoints()
		ZGV.Frame:GetParent():SetPoint("TOPLEFT",MerchantFrame,"TOPRIGHT")
	end
end
...
function IM:DetachViewerVendor()
	...
	if ZGV.db.profile.frame_anchor_prevendor then
		ZGV.db.profile.frame_anchor_prevendor[2]=UIParent
		ZGV.Frame:GetParent():ClearAllPoints()
		ZGV.Frame:GetParent():SetPoint(unpack(ZGV.db.profile.frame_anchor_prevendor))
		ZGV.db.profile.frame_anchor_prevendor = nil
	end
end
...
function IM:GetUnusableItems()
	...
	for bagID=0, NUM_BAG_SLOTS do
		for bagSlotID=1,GetContainerNumSlots(bagID) do repeat
			if not (GetContainerItemEquipmentSetInfo and GetContainerItemEquipmentSetInfo(bagID,bagSlotID)) then -- don't sell equipment sets
				local itemLink = GetContainerItemLink(bagID,bagSlotID)
				if itemLink then
