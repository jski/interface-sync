...
function Upgrades:ScoreBagsItems()
	...
	for itemlink,item in pairs(Upgrades.BagsItems) do
		if item.itemlink then
			itemlink = strip_link(itemlink)
			if (item.class == LE_ITEM_CLASS_WEAPON and (ZGV.IsRetail or not (item.type=="INVTYPE_THROWN" or item.type=="INVTYPE_RANGED" or item.type=="INVTYPE_RANGEDRIGHT"))) or (item.type=="INVTYPE_HOLDABLE" or item.type=="INVTYPE_SHIELD") then
